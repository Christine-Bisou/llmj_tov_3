...
--
-- Пара берётся НЕУПОРЯДОЧЕННОЙ (min/max по сорсам): в обратном прогоне ответы
-- переставлены, и упорядоченный ключ (A,B) не нашёл бы там (B,A).
...
--
-- Если в обратной таблице нет source_A/source_B — соберите ключ из ответов:
...
$keyed_direct = (
    SELECT
        i.*,
        $pair_key(i.instruct_id, i.source_A, i.source_B) AS pair_key
    FROM $input1 AS i
);
