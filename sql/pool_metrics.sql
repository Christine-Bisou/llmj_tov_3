...
$all_rows = (
    SELECT
        a.*,
        -- Сколько всего пар в прогоне: по нему решаем, откуда брать имена
        -- моделей и нужен ли попарный срез вообще.
        p.cnt AS pair_cnt,
...
-- Все три уровня уходят в один выход, различаются колонкой level:
--   input       — общая строка по прогону с именами моделей со входа;
--   source      — попарно по source_A/source_B;
--   real_source — попарно по настоящим именам моделей.
-- Попарные строки по source пишем только когда пар в прогоне больше одной:
-- при единственной паре (то есть двух source на всю таблицу) такой срез
-- считается по тем же заданиям и с теми же _target, что и строка input,
-- и отличается от неё только подписью моделей — дубль не нужен.
INSERT INTO $output1
SELECT
    t.*
WITHOUT t.is_mapped, t._norm_m1, t._pair_cnt
FROM $final_metrics AS t
WHERE (t.is_mapped = 0 AND t._norm_m1 = "ALL")
   OR (t.is_mapped = 0 AND t._norm_m1 != "ALL" AND t._pair_cnt > 1)
   OR (t.is_mapped = 1 AND t._norm_m1 != "ALL");
