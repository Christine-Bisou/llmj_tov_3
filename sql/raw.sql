...
SELECT
    ...
    -- Из обвязки отдельной колонкой идёт только rownum: по нему сортируют и
    -- сверяют выгрузки. Тикет, корзина, пул и проект лежат в markup_metadata,
    -- дублировать их колонками смысла нет.
