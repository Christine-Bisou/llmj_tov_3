...
SELECT
  ...
  -- ключ для склейки двух выходов: номер строки во входной таблице.
  -- Оба INSERT читают один и тот же $input1 без сортировок и джойнов,
  -- поэтому одной строке соответствует одинаковый индекс в обоих выходах.
  TableRecordIndex() AS for_join,

  -- WITHOUT обязан быть последним элементом списка, иначе YQL ругается
  t.* WITHOUT IF EXISTS t.answer_slot, t.tov_prompt, t._other, t.infer_dialog, t.for_join
FROM $input1 AS t;
