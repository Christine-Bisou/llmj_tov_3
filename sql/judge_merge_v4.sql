...
--
-- Ключ джойна — instruct_id ВМЕСТЕ с парой моделей. По одному instruct_id
-- строк больше одной (один и тот же диалог сравнивают в разных парах),
-- и джойн только по нему давал декартово произведение: каждая строка слева
-- склеивалась с каждой одноимённой справа, отсюда рост числа строк.
$parsed = (
    SELECT
        i1.*,
