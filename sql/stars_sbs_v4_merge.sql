...
--
-- РАСКЛАДКА ПО КОЛОНКАМ ЭТОЙ ТАБЛИЦЫ (важное отличие от прошлой версии):
--   clarity_N / liveliness_N / connect_N / overall_N — плоские int64, уже есть
--       в схеме рядом с золотом (*_gs). Итоговые звёзды кладём именно туда,
--       старые значения снимаем через WITHOUT, иначе «Duplicated member».
--   pointwise_N — подробности по проходам и обоснования (any).
--   markers_N   — маркеры ПОСЛЕ аудита; markers_N_answer не трогаем, это вход
--       первого этапа, он нужен для сверки «что джадж поменял на аудите».
--   answer_source_1 / answer_source_2 — источники (раньше были source_A/B).
--   golden_winner и *_gs проходят насквозь: по ним считается качество.
--
-- Схема таблицы приехала обрезанной, поэтому весь WITHOUT — с IF EXISTS:
-- лишнее имя в списке безвредно, недостающее уронит запрос.
--
-- Учти: sql/error_breakdown.sql читает золото из колонки winner, а здесь оно
-- называется golden_winner — при запуске метрики поправь имя там.
...
-- 0.0 здесь означает «прохода нет или он не распарсился» — оценки от 1 до 5.
...
-- Если один проход не распарсился, среднее с нулём занизило бы звезду вдвое.
-- Поэтому берём уцелевший проход как есть, а «нет ни одного» отдаём нулём.
$avg = ($dir, $rev, $md, $mr, $asp) -> {
    $d = $score($dir, $md, $asp);
    $r = $score($rev, $mr, $asp);
    RETURN CASE WHEN $d > 0.0 AND $r > 0.0 THEN ($d + $r) / 2.0
                WHEN $d > 0.0              THEN $d
                WHEN $r > 0.0              THEN $r
                ELSE 0.0 END;
};
...
-- Версия для плоских колонок: NULL вместо нуля, чтобы битые строки не тянули
-- вниз AVG при сравнении с *_gs.
$star_opt = ($dir, $rev, $md, $mr, $asp) -> {
    $a = $avg($dir, $rev, $md, $mr, $asp);
    RETURN IF($a > 0.0, Just(CAST(Math::Floor($a) AS Int64)), Nothing(Int64?));
};
...
-- Четыре числа без обвязки — формат разметки. Дублирует плоские колонки,
-- но его ждёт скрипт выгрузки в разметку, поэтому оставляем.
$clc = ($dir, $rev, $md, $mr) -> {
    RETURN Just(Yson::From(<|
...
$calc = (
    SELECT
        ...
        -- звёзды и вердикт этой строки считаем заново, поэтому снимаем всё,
        -- что уже лежало в таблице с прошлых прогонов: иначе алиасы ниже
        -- упрутся в «Duplicated member». Звёздочка с WITHOUT — строго последняя
        -- в списке: после неё парсер ждёт только имена колонок.
        p.* WITHOUT IF EXISTS
            p.tov_winner, p.tov_winner_source,
            p.clarity_1, p.clarity_2,
            p.liveliness_1, p.liveliness_2,
            p.connect_1, p.connect_2,
            p.overall_1, p.overall_2,
            p.pointwise_1, p.pointwise_2,
            p.parsed_ok
    FROM $parsed AS p
);
...
SELECT
    -- ---------- звёзды: плоско, рядом с золотом *_gs ----------
    -- имя колонки золота для живости — liveness_*_gs, без второй «li»;
    -- это опечатка в схеме, а не другой аспект, оценку кладём в liveliness_N
    $star_opt(f.dir_yson, f.rev_yson, 'model_1_evaluation', 'model_2_evaluation', 'clarity')    AS clarity_1,
    $star_opt(f.dir_yson, f.rev_yson, 'model_1_evaluation', 'model_2_evaluation', 'liveliness') AS liveliness_1,
    $star_opt(f.dir_yson, f.rev_yson, 'model_1_evaluation', 'model_2_evaluation', 'connect')    AS connect_1,
    $star_opt(f.dir_yson, f.rev_yson, 'model_1_evaluation', 'model_2_evaluation', 'overall')    AS overall_1,

    $star_opt(f.dir_yson, f.rev_yson, 'model_2_evaluation', 'model_1_evaluation', 'clarity')    AS clarity_2,
    $star_opt(f.dir_yson, f.rev_yson, 'model_2_evaluation', 'model_1_evaluation', 'liveliness') AS liveliness_2,
    $star_opt(f.dir_yson, f.rev_yson, 'model_2_evaluation', 'model_1_evaluation', 'connect')    AS connect_2,
    $star_opt(f.dir_yson, f.rev_yson, 'model_2_evaluation', 'model_1_evaluation', 'overall')    AS overall_2,

    -- ---------- звёзды: подробности по проходам ----------
    ...
    -- ---------- маркеры после аудита ----------
    -- markers_1_answer / markers_2_answer остаются нетронутыми: это разметка
    -- первого этапа, по ней видно, что аудит второго этапа снял или добавил
    ...
    CAST(f.tov_winner AS String)                                        AS tov_winner,
    $as_source(f.tov_winner, f.answer_source_1, f.answer_source_2)      AS tov_winner_source,

    -- строка годна к подсчёту метрик, только если распарсились ОБА прохода:
    -- на одном проходе нет ни склейки вердикта, ни усреднения звёзд
    (f.dir_yson IS NOT NULL AND f.rev_yson IS NOT NULL)                 AS parsed_ok,

    Just(Yson::From(<|
        model_winner_direct:              f.w_direct,
        model_winner_reversed_normalized: f.w_reversed_norm,
        parsed_direct:                    f.dir_yson IS NOT NULL,
        parsed_reversed:                  f.rev_yson IS NOT NULL,
        reasoning_direct:                 $sbs_why(f.dir_yson),
        reasoning_reversed:               $sbs_why(f.rev_yson),
    ...
    -- Первый блок — служебное этого запроса, второй — сырой вход джаджа
    -- (он большой и уже разобран), третий — колонки, которые мы только что
    -- пересчитали: они есть во входной таблице с прошлых этапов, и без снятия
    -- YQL падает с «Duplicated member».
    f.* WITHOUT IF EXISTS
        ...
        f.tov_winner, f.tov_winner_source, f.parsed_ok,
        f.clarity_1, f.clarity_2,
        f.liveliness_1, f.liveliness_2,
        f.connect_1, f.connect_2,
        f.overall_1, f.overall_2,
