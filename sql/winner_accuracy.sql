...
--
-- $output2 — тот же расчёт для «броска монеты». Если модель не обгоняет
-- бейзлайн заметно, разбираться надо не с промтом, а с разметкой.
...
SELECT
    ...
    -- распределение: сколько ничьих с каждой стороны
...
SELECT
    ...
    -- контроль: если вердикт не распознан, здесь будет не 0 и метрики выше
    -- посчитаны не по всей таблице
