...
$prep = (
    SELECT
        ...
        real_source_B,
        rownum,
        -- Всю обвязку задания достаём из markup_metadata: отдельными колонками
        -- первый этап её больше не пишет, там осталась только rownum.
        ...
        $str_yson(markup_metadata.project_id) AS project_id,
        assignment_ids,
        metadata,
        markers,
        annotations,
...
$input4_prep = (
    SELECT
        ...
        Just(input_final_messages) AS input_final_messages,
        Just(input_meta) AS input_meta,
...
SELECT
    ...
    -- Обвязка задания живёт в markup_metadata, отдельными колонками её здесь
    -- нет. Исключение — rownum: по нему сортируют и сверяют выгрузки.
    rm.rownum AS rownum,
