...
--
-- Схема входа (одинаковая у обеих таблиц):
--   answer_1, answer_2, answer_source_1, answer_source_2, dialog, meta,
--   instruct_id, dst, reasoning_dst, infer_dialog, parsed_ok,
--   clarity_1/2, liveliness_1/2, connect_1/2, overall_1/2, pointwise_1/2,
--   markers_1_answer, markers_2_answer,
--   model_1_analysis, model_2_analysis, model_1_linguistic_scan, model_2_linguistic_scan
--
-- Плоские звёзды (clarity_*, liveliness_*, connect_*, overall_*), pointwise_*
-- и parsed_ok во входе относятся к ОДНОМУ проходу. Здесь они пересчитываются
-- по обоим и перекрывают входные — старые версии снимаются в WITHOUT, иначе
...
--   clarity_N / liveliness_N / connect_N / overall_N — те же звёзды плоско;
...
SELECT
    ...
    -- те же звёзды плоско: во входе они были по одному проходу, здесь — по двум
    ...
    -- строка считается разобранной, только если распарсились ОБА прохода:
    -- при пустом rev_yson все reversed-звёзды молча стали бы нулями
    ...
    -- Первый блок — служебное этого запроса, второй — колонки входа, которые мы
    -- только что пересчитали: без снятия YQL падает с «Duplicated member».
    f.* WITHOUT IF EXISTS
        f.dir_yson, f.rev_yson,
        f.mk1_dir, f.mk2_dir, f.mk1_rev, f.mk2_rev,
        f.w_direct, f.w_reversed_norm,
        f.dst, f.reasoning_dst, f.infer_dialog, f._other,
